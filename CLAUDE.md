...
- **Codex-specific local development:** when Codex needs to start the app stack with the repo `script/*.sh` launchers, it must open one dedicated macOS Terminal window/tab and run `script/start_all.sh` there, rather than invoking the scripts directly inside Codex's managed command executor. In this environment, `nohup ... > logs/*.log 2>&1 &` children started from the managed executor are reaped when the executor exits, so services appear to start, `logs/*.log` may be created or truncated, but `uvicorn` / `celery` / `vite` do not stay alive. Running the same script from a real Terminal session keeps the processes attached to that Terminal instead of the Codex executor, allowing them to remain up and continue writing to `logs/backend.log`, `logs/frontend.log`, `logs/celery_ai.log`, and `logs/celery_video.log`.
