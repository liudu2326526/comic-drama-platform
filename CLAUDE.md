...
**Project-level visual reference workflow**:

- `character_style_reference_*` and `scene_style_reference_*` live on `projects` and are generated by async jobs (`gen_character_style_reference`, `gen_scene_style_reference`). Routes must return a job ack immediately; the frontend must use `useJobPolling` and `loadCurrent` re-attachment rather than blocking the click handler.
- The character style reference is only an input to concrete character asset generation (`gen_character_asset` full-body/headshot). The scene style reference is only an input to concrete scene asset generation (`gen_scene_asset`). Do not cross-feed the scene reference into character generation or the character reference into scene generation unless a new spec explicitly changes that contract.
- Style references must be generated before the concrete assets that should use them. Creating or regenerating a project-level style reference does **not** automatically rewrite already generated character/scene assets; users must explicitly regenerate the concrete assets if they want the new reference applied.
- Stage gating is intentional: character style reference actions are editable during `storyboard_ready`; scene style reference actions are editable during `characters_locked`. Once the project advances to `scenes_locked`, scene style reference buttons should be disabled unless the user rolls back to an editable stage.
...
| 40901 | 409  | business conflict (e.g. duplicate in-flight character/scene generation) |
...
- **Codex-specific local development:** when Codex needs to start the app stack with the repo `script/*.sh` launchers, it must use the reusable dev terminal helper: run `script/dev_terminal.sh open` to create or reuse the fixed `tmux` session `comic-drama-dev` and attach it in macOS Terminal, rather than invoking `script/start_all.sh` directly inside Codex's managed command executor. If `tmux` is missing, the helper installs it with Homebrew before creating the session. After that, send follow-up commands into the same terminal with `script/dev_terminal.sh send "<command>"`, inspect it with `script/dev_terminal.sh status`, or attach manually with `script/dev_terminal.sh attach`; do not open a fresh Terminal tab unless the helper reports that no client is attached. In this environment, `nohup ... > logs/*.log 2>&1 &` children started from the managed executor are reaped when the executor exits, so services appear to start, `logs/*.log` may be created or truncated, but `uvicorn` / `celery` / `vite` do not stay alive. Running `script/start_all.sh` inside the reusable `tmux`/Terminal session keeps the processes attached outside the Codex executor, allowing them to remain up and continue writing to `logs/backend.log`, `logs/frontend.log`, `logs/celery_ai.log`, and `logs/celery_video.log`.
...
- Batch AI workflows that create multiple children (storyboards, character assets, scene assets, regenerations) should fan out into concurrent Celery child jobs where possible. Parent jobs must derive progress from terminal child states and must fail if any child fails/cancels; otherwise the UI can sit forever in a misleading running state.
